...
function TrainingConstructor() {
    const dispatch = useDispatch();
    const history = useHistory();
    const {id: vocabularyId} = useParams();
    const trainingName = 'constructor';
        ...
        <MainContainer maxWidth="700px">
            {!isCompleted && (
                <TrainingQuiz
                    ...
                    letterGuessed={words[step-1].letterGuessed}
                    letterMistake={words[step-1].letterMistake}
                    letterOptions={words[step-1].letterOptions}
                />
            )}
            {isCompleted && (
                <TrainingResults
